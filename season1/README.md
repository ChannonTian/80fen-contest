...
2 → A,过 `2/5/10/13` 四道关。裁判引擎是线上正式版。
...
第一赛季带着两处跑完了,都写在 `RULES.md` §J 里,这一季的记录里都能查到影响面:

1. **`decompose` 的贪心断链** —— 拆牌型时贪心扫描会漏掉某些拖拉机的拆法。
2. **领出非法牌没有判罚** —— 领出混门(比如 ♥5+♦7)会让裁判自己抛异常,整场作废、
   双方都不计。等于给「打不过就掀桌」留了一条出路。

**本季五份提交一次都没触发过**(零违规、零崩溃),所以两处都排期在本季之后修 ——
中途改判罚会让已跑的 258722 局失去共同基准。第二赛季开跑前会修掉。
