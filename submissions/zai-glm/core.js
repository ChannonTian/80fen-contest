...
const AIP = {                 // v3 可调参数,全部集中在这里便于调参
  ...
  egPointsEps: 0.008,
  /* ---- zai-glm s2 增量(全部默认关;index.js 里打开) ---- */
  s2World: 0,      // 推演策略改用「世界内信息」(各家手牌),不再把搜索者的真实记牌发给模拟玩家
  s2AdvanceU: 0,   // 效用函数改用 advanceMatch 实算级数(关卡/played 感知)
  s2Depth: 0,      // >0 时覆盖 egMaxCards(更深搜索)
  s2Cands: 0,      // >0 时覆盖 egMaxCands
  s2SamplesBy: 0,  // >0 时按深度衰减样本数:samples = max(24, egSamples - s2SamplesBy*(n-5))
  s2Wsample: 0,    // 世界采样按软断门概率加权(pVoidOf),不再按空位均匀
  s2Wkappa: 1.0,   // 权重 = 空位 × (1-pVoid)^kappa
  s2LadderU: 0,    // scorePlay 的 ev 加混合式阶梯跨线加成(纯替换实测 −8.5:台阶内部激励归零)
  s2LadderScale: 40, // 一级台阶 = 多少分当量
  s2LadderMix: 0.3,  // 阶梯加成混合系数
  s2Exact: 0,       // 收官搜索:各家 ≤s2ExactMax 张时用完全信息 minimax 精确求解替代走子
  s2ExactMax: 3,    // 每家手牌上限(2=只解最后两墩,3=更准更贵)
  s2Kitty: 0,       // 防守方视角:底牌按庄家扣底策略加权采样(不埋主/少埋分),不再当均匀溢出桶
  s2KittyTrump: 0.05, // 底牌里主牌的相对权重(faceValue 400 阻力 ≈ 庄家几乎不埋主)
  s2KittyPts: 0.4,    // 底牌里分牌的相对权重(PointΔ 多为负 ≈ 少埋分)
...
/* s2:把「闲家拿到 x 分」折成我方级数当量(只走 0/40/80/120/160/200 这排台阶,
 * 与 scoreRound 的升档一致)。iAmDef = 我是闲家方。 */
function ladderU(defPts, iAmDef){
  let my=0, opp=0;
  if(defPts>=80){ const up=1+Math.min(3,Math.floor((defPts-80)/40)); if(iAmDef) my=up; else opp=up; }
  else { const up=defPts===0?3:defPts<40?2:1; if(iAmDef) opp=up; else my=up; }
  return my-opp;
}
function scorePlay(X, cards, cat, beatsCur){
  ...
  if(AIP.s2LadderU&&X.def!==undefined){
    /* 混合式阶梯加成:线性 ev 保留为基底(台阶内部的分数激励不能丢 —— 纯阶梯差分在
     * 40 分宽的台阶内部恒为 0,单墩 delta 极少跨线,全局替换会把中盘评分变瞎,实测 −8.5 级),
     * 阶梯差分只作为「跨线敏感性」附加项 ×s2LadderMix。 */
    const iDef=!X.isDecl;
    const dWin=X.def+(iDef?gain:0);
    const dLose=X.def+(iDef?0:loss);
    const lad=p*ladderU(dWin,iDef)+(1-p)*ladderU(dLose,iDef)-ladderU(X.def,iDef);
    const ev=(p*gain-(1-p)*loss)*(X.pw||1)+AIP.s2LadderMix*AIP.s2LadderScale*lad;
    return (CAT_K[cat]||1)*ev+tempo-futureValue(cards,X);
  }
...
function scoreLeadEV(L, cl, cards, pre){
  ...
  let s;
  if(AIP.s2LadderU&&L.def!==undefined){
    const iDef=!L.isDecl;
    const dWin=L.def+(iDef?gain:0);
    const dLose=L.def+(iDef?0:loss);
    const lad=p*ladderU(dWin,iDef)+(1-p)*ladderU(dLose,iDef)-ladderU(L.def,iDef);
    s=(p*gain-(1-p)*loss)*(L.pw||1)+AIP.s2LadderMix*AIP.s2LadderScale*lad;
  }else s=(p*gain-(1-p)*loss)*(L.pw||1);
  s-=fut;
  let reason;
...
function sampleWorld(ctx, rand){
  ...
  for(let attempt=0;attempt<AIP.egRetries;attempt++){
    ...
    for(const c of order){
      ...
      // 按剩余空位加权,避免最后一家被塞满;s2 时再乘「他持有这门」的推断概率 ——
      // 已知/演绎出他缺这门的概率越高,这张牌落他手里的权重越低(BDCI 思路)
      // s2Kitty:底牌作为伪座位参与加权(只对不知道底牌的一方生效),权重 ∝ 扣底策略倾向
      let tot=0; const w=cand.map(s=>{ let x=sizes[s]-hands[s].length;
        if(ctx.pVoidOf){ const pv=ctx.pVoidOf(s,su); if(pv>=1) return 0;
          x*=Math.pow(1-pv, AIP.s2Wkappa); }
        tot+=x; return x; });
      let wk=0, kittyRoom=kittySize-kitty.length;
      if(ctx.kittyProp&&kittyRoom>0){ wk=ctx.kittyProp(c)*kittyRoom*AIP.s2KittySeat; tot+=wk; }
      let r=rand()*tot, pick=cand[0];
      if(wk>0){ r-=wk; if(r<=0){ kitty.push(c); continue; } }
      for(let i=0;i<cand.length;i++){ r-=w[i]; if(r<=0){ pick=cand[i]; break; } }
      if(tot<=0) pick=cand[(rand()*cand.length)|0];   // 全零权重(不该发生)兜底
...
/* ---- zai-glm s2:精确收官求解(AIP.s2Exact) ----
 * rollout 打到各家剩 ≤s2ExactMax 张时,剩余墩数 ≤2-3,改用完全信息 minimax
 * (alpha-beta)精确解 —— 末两墩正是「底分×倍数」的决策点,走子的噪声在这里最贵。
 * 着法枚举:领出 = 手牌全部子集里 classify 非 null 的(≤2^k−1 个);跟牌 = 同长度
 * 子集过 isLegalFollow(≤ C(k,n) 个),天然合法,量级在手牌 ≤3 时完全可行。 */
function subsetsOf(hand){
  const out=[], n=hand.length, cur=[];
  const rec=start=>{
    if(cur.length) out.push(cur.slice());
    for(let i=start;i<n;i++){ cur.push(hand[i]); rec(i+1); cur.pop(); }
  };
  rec(0);
  return out;
}
function leadOptions(hand, trump, hands, seat){
  const seen=new Set(), out=[];
  for(const cards of subsetsOf(hand)){
    const cl=classify(cards,trump);
    if(!cl) continue;
    let play=cards;
    if(cl.type==='throw'){
      const chk=checkThrow(hands, seat, cards, trump);
      if(!chk.ok) play=chk.forced;   // 甩牌被吃:裁判只让出最小组(裁判口径)
    }
    const k=play.map(c=>c.id).sort((a,b)=>a-b).join(',');
    if(!seen.has(k)){ seen.add(k); out.push(play); }
  }
  return out;
}
function followOptions(hand, lead, trump){
  const n=lead.cards.length, out=[];
  if(hand.length<n) return out;
  const cur=[];
  const rec=start=>{
    if(cur.length===n){
      const cards=cur.slice();
      if(isLegalFollow(hand,lead,cards,trump)) out.push(cards);
      return;
    }
...
/* 完全信息求解:返回「闲家最终总分(含底翻)」。闲家队 max、庄家队 min;
 * trick 内四手顺序展开,跨着法 alpha-beta。 */
function solveTail(hands, leader, defPoints, trump, declSeat, kittyPts){
  const defTeam=1-declSeat%2;
  const terminal=(defPts,lastWinner,lastLeadSize)=>
    defPts+(lastWinner%2===defTeam?kittyPts*2*lastLeadSize:0);
  function trick(st, plays, alpha, beta){
    if(plays.length===4){
      const res=resolveTrick(plays,trump);
      const def2=st.def+(res.winner%2===defTeam?res.points:0);
      const L=plays[0].cards.length;
      if(st.hands.every(h=>h.length===0))
        return terminal(def2,res.winner,L);
      return trick({hands:st.hands,leader:res.winner,def:def2}, [], alpha, beta);
    }
    const seat=plays.length?(plays[0].seat+plays.length)%4:st.leader;
    const hand=st.hands[seat];
    const lead=plays.length===1?classify(plays[0].cards,trump):null;
    const opts=lead?followOptions(hand,lead,trump):leadOptions(hand,trump,st.hands,seat);
    if(!opts.length) return st.def;   // 不该发生
    const maxing=seat%2===defTeam;
    let best=maxing?-1e9:1e9;
    for(const o of opts){
      const ids=new Set(o.map(c=>c.id));
      const hands2=st.hands.map((h,i)=>i===seat?h.filter(c=>!ids.has(c.id)):h);
      const v=trick({hands:hands2,leader:st.leader,def:st.def}, plays.concat([{seat,cards:o}]), alpha, beta);
  ...
  return trick({hands:hands.map(h=>h.slice()),leader,def:defPoints}, [], -1e9, 1e9);
}
/* ---- zai-glm s2:世界内推演(AIP.s2World) ----
 * 原版把**搜索者的真实记牌** st.mem 发给推演里的每个玩家 —— 等于把我方信息
 * 泄露给模拟对手/队友,系统性高估对手应对质量。改为:推演玩家只用世界内信息
 * (世界 = 采样出的各家手牌;底牌永远不出场,不参与「压得住」判断)。
 * 领出侧:钢板判断对着世界内手牌(增量缓存:每门最大单张/最大对子,脏标记懒重算);
 * 跟牌侧:队友暂大时,那手是否世界内无敌决定贴分还是跟小。
 * 策略保持便宜 —— 样本量仍然优先于单样本质量。 */
function makeWorldCtx(trump, hands){
  const dirty={T:1,S:1,H:1,D:1,C:1}, maxS={}, maxP={};
  const suitMax=su=>{
    if(!dirty[su]) return;
    let ms=-1, mp=-1;
    for(const h of hands){
      const cnt={};
      for(const c of h){
        if(effSuit(c,trump)!==su) continue;
        const o=ordIdx(c,trump);
        if(o>ms) ms=o;
        const k=c.suit+':'+c.rank;
        cnt[k]=(cnt[k]||0)+1;
        if(cnt[k]>=2&&o>mp) mp=o;
      }
    }
    maxS[su]=ms; maxP[su]=mp; dirty[su]=false;
  };
  return {
    trump, hands,
    onRemove:c=>{ dirty[effSuit(c,trump)]=true; },
    boss(cl){ suitMax(cl.suit);
      return (cl.type==='single'?maxS[cl.suit]:maxP[cl.suit])<=cl.top; },
  };
}
function rolloutLead2(hand, trump, w){
  const bySuit={};
  hand.forEach(c=>{(bySuit[effSuit(c,trump)]=bySuit[effSuit(c,trump)]||[]).push(c);});
  let best=null, bestKey=-1e9;
  for(const su in bySuit){
...
function rolloutFollow2(hand, lead, trump, plays, seat, w){
  const cur=currentWinner(plays,trump);
  const partnerWinning=cur.seat%2===seat%2;
  const isLast=plays.length===3;
  const pts=countPoints(plays.flatMap(p=>p.cards));
  if(partnerWinning){
    // 队友暂大:世界内无敌(或我已是末家)就贴分,否则跟小
    const sure=isLast||w.boss(cur.cl);
    return buildFollow(hand,lead,trump,sure?'dump':'cheap',null)||buildFollow(hand,lead,trump,'cheap',null);
  }
  const win=(pts>0||isLast)?minWinFollow(hand,lead,trump,cur.cl,isLast):null;
  if(win) return win;
  return buildFollow(hand,lead,trump,'cheap',null)||buildFollow(hand,lead,trump,'discard',null);
}
/* s2 效用:用 advanceMatch 实算两级级数差 —— 关卡钳制(clampAtGate)与 played
 * 都在真实推进里,「卡在关卡上的升级」不再被记作进展。 */
function levelUtility2(sc, myTeam, declTeam, mm){
  const adv=advanceMatch(mm.levels.slice(), mm.declSeat, sc, mm.gates&&mm.gates.length?mm.gates:null, mm.played.slice());
  const my=(adv.levels[myTeam]-mm.levels[myTeam])-(adv.levels[1-myTeam]-mm.levels[1-myTeam]);
  const myPts=declTeam===myTeam?(200-sc.total):sc.total;
  return my+AIP.egPointsEps*myPts;
}

/* 从当前局面推演到局末,返回本局闲家总分与最后一墩信息 */
/* s2TeamMax:我方席位在推演里的增强策略(cooperative determinization 的落地版)。
 * 只在「底分>0 且各家 ≤s2TeamMaxEnd 张」的收官段生效:
 *  - 跟牌:末家能赢就赢;对手暂大时,台面有分或要抢牌权就吃进来(原策略只在有分时吃);
 *    队友暂大保持原样(贴分/跟小,避免无谓压队友)。
 *  - 领出:只领钢板 —— 闲家方挑「组最大」的钢板(赢了就是抠底倍数),
 *    庄家方挑「最稳的小组」(守住即可,倍数无关)。 */
function teamFollow(hand, lead, trump, plays, seat, st){
  const cur=currentWinner(plays,trump);
  const partnerWinning=cur.seat%2===seat%2;
  const isLast=plays.length===3;
  const pts=countPoints(plays.flatMap(p=>p.cards));
  const endgame=st.hands.every(h=>h.length<=AIP.s2TeamMaxEnd);
  if(isLast){
    const w=minWinFollow(hand,lead,trump,cur.cl,true);
    if(w) return w;
    return buildFollow(hand,lead,trump,'cheap',null)||buildFollow(hand,lead,trump,'discard',null);
  }
  if(!partnerWinning){
    const wantLead=pts>0||(st.kittyPts>0&&endgame);
    if(wantLead){
      const w=minWinFollow(hand,lead,trump,cur.cl,false);
      if(w) return w;
    }
  }
  return null;   // 其余情形交回原策略
}
function teamLead(hand, trump, mem, st){
  const endgame=st.hands.every(h=>h.length<=AIP.s2TeamMaxEnd);
  if(!(st.kittyPts>0&&endgame)) return null;
  const iDef=st.myTeam!==undefined&&st.myTeam!==st.declSeat%2;
  const bySuit={};
  hand.forEach(c=>{(bySuit[effSuit(c,trump)]=bySuit[effSuit(c,trump)]||[]).push(c);});
  let best=null, bestKey=-1e9;
  for(const su in bySuit){
    for(const comp of decompose(bySuit[su],trump)){
      const cl={type:comp.type,len:comp.len,suit:su,top:comp.top};
      if(!isBossPlay(cl,mem,trump)) continue;
      const size=comp.cards.length, p=countPoints(comp.cards);
      const k=iDef? size*10+p*2 : 40+p*2-size*3;
      if(k>bestKey){ bestKey=k; best=comp.cards; }
    }
  }
  return best;
...
function rolloutToEnd(st){
  ...
  let guard=0;
  const world=AIP.s2World?makeWorldCtx(trump,hands):null;
  const drop=seat=>cards=>cards.forEach(c=>{removeCard(hands[seat],c); if(world)world.onRemove(c);});
  const teamMax=AIP.s2TeamMax&&st.myTeam!==undefined;
  while(hands.some(h=>h.length)||plays.length){
    if(plays.length===0){
      // s2 精确收官:剩 ≤s2ExactMax 张时,余下各墩完全信息精确解(含底翻),
      // 替代走子 —— 末两墩是底分×倍数的决策点,噪声最贵。
      if(AIP.s2Exact&&st.exactOK&&hands.every(h=>h.length>0&&h.length<=AIP.s2ExactMax))
        return {exactTotal:solveTail(hands,leader,defPoints,trump,declSeat,st.kittyPts||0)};
      let cards;
      if(teamMax&&leader%2===st.myTeam){
        cards=teamLead(hands[leader],trump,st.mem,st)
             ||(world?rolloutLead2(hands[leader],trump,world):rolloutLead(hands[leader],trump,st.mem));
      }else cards=world?rolloutLead2(hands[leader],trump,world):rolloutLead(hands[leader],trump,st.mem);
      drop(leader)(cards);
...
function endgameSearch(view, plays, cands){
  ...
  const pVoidOf=AIP.s2Wsample?makeVoidProb(reads,mem,trump,view.hand.length):null;
  /* s2Kitty:防守方(不知道底牌)采样时,底牌按庄家扣底策略加权 —— 庄家几乎不埋主
   * (faceValue 400 阻力)、少埋分(PointΔ 多为负)。底牌作为「伪座位」参与加权分配,
   * 而不是只收没人要的溢出牌 —— 这改变闲家的抠底期望,直接影响末墩争夺的取舍。 */
  const kittyProp=(AIP.s2Kitty&&kittySize>0)?c=>{
    if(effSuit(c,trump)==='T') return AIP.s2KittyTrump;
    return cardPoints(c)>0?AIP.s2KittyPts:1;
  }:null;
  const rand=rng((view.history||[]).length*7919+view.seat*31+n*17+1);
  const ctx={unseen,sizes,seats,voids,trump,kittySize,capOf,pVoidOf,kittyProp};
  const myTeam=view.seat%2, declTeam=view.declSeat%2;
  const mm=AIP.s2AdvanceU?{levels:view.levels.slice(),played:(view.played||[-1,-1]).slice(),
                           gates:view.gates||[],declSeat:view.declSeat}:null;
  const rs=roundScore(view);
  ...
  for(const w of worlds){
    ...
    for(const o of out){
      ...
      const r=rolloutToEnd({hands,plays:pl,leader,defPoints,trump,declSeat:view.declSeat,mem,rand,
                            kittyPts:countPoints(kitty),exactOK:n<=AIP.s2ExactMax,myTeam:view.seat%2});
      const sc=r.exactTotal!==undefined
        ?scoreRound({defPoints:r.exactTotal,kitty:[],defWonLastTrick:false,lastLeadSize:1})
        :scoreRound({defPoints:r.defPoints,kitty,
...
const ENGINE={RULES,makeDeck,cardPoints,countPoints,rng,dealRound,
  ...
  endgameSearch,sampleWorld,rolloutToEnd,solveTail,leadOptions,followOptions,rolloutLead,rolloutFollow,levelUtility,trumpLeadPrior};
