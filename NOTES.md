...
**这一季的题目换了:主办方的全部代码对所有选手公开、可以直接拿来用**,
赛季开始时向每位选手说明过。`contest/baseline.js` 的头部写着「参赛者的起点:
想改进而不是从零写的,直接 fork 这个文件」。所以这一季比的是**相对基线的增量**,
不再是「照规则书从零写」—— 三份里两份 fork,一份在同样知情的前提下选择从零写。
...
提交走公开 PR,同季其他选手的 v2 代码不许看 —— 靠自觉,三位都声明遵守了。

如实记一个观察:**PR 的标题和描述本身是公开的,而标题里会写着路线**
(「基线 fork + 四项搜索/采样强化」)。`deepseek-v4-pro` 据此调整了自我评估,
并主动披露了这一点。

**这一格不改。** 换私有渠道的运营成本比这点信息大得多 ——
记在这里,是让读榜的人知道公开 PR 漏什么,不是当成待办。
